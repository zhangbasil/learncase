package com.zhangbin.learncase.excel.util;
...
import com.zhangbin.learncase.excel.annotation.IgnoreField;
import com.zhangbin.learncase.excel.annotation.StaticExcelRow;
import com.zhangbin.learncase.excel.bean.ExcelRowCellInfo;
import com.zhangbin.learncase.excel.bean.ExportCellStyleInfo;
import com.zhangbin.learncase.excel.bean.ExportFieldInfo;
import com.zhangbin.learncase.excel.bean.ExportInfo;
import com.zhangbin.learncase.excel.bean.ImportFieldInfo;
import com.zhangbin.learncase.excel.bean.ImportInfo;
import com.zhangbin.learncase.excel.bean.SortComparator;
import com.zhangbin.learncase.excel.bean.SortableField;
import com.zhangbin.learncase.excel.bean.StaticExcelRowCellInfo;
import com.zhangbin.learncase.excel.bean.inter.ExportProcessor;
import com.zhangbin.learncase.excel.bean.inter.ImportProcessor;
...
public class ExcelUtils {
    ...
    public static ExportInfo getExportInfo(Class<?> clazz,Map<String,String> map) throws IntrospectionException {
...
//            maxSheetSize = Integer.parseInt(map.get("sheetSize"));
        ...
        //
        exportInfo = new ExportInfo(sheetName, headRowNum, dataRowNum, fieldInfoMap, sortFieldList);
    ...
    private static int createExportFieldInfo(Class<?> clazz, ExportCellStyleInfo parentHeadStyle,
            ExportCellStyleInfo parentDataStyle, Map<Field, ExportFieldInfo> fieldInfoMap, List<Field> fieldList,
            List<SortableField> sortFieldList, List<Method> parentMethods, int count) throws IntrospectionException {
        for (int i = 0; i < fieldList.size(); i++) {
            ...
            if (!ClassUtils.isSimpleType(field.getType())) {
                ...
                int temp = createExportFieldInfo(field.getType(), fieldHeadStyleInfo, fieldDataStyleInfo, fieldInfoMap,
                        ClassUtils.getAllFieldFromClass(field.getType()), list, methodChain, count + 1);
    ...
    private static void setExcelFieldInfo(ExportFieldInfo fieldInfo, ExcelField excelField) {
    ...
    private static void createImportFieldInfo(List<Class<?>> clazzChain, List<Method> setMethodChain,
            List<Method> getMethodChain, Map<String, ImportFieldInfo> fieldInfoMap, List<Field> fields)
        ...
        for (Field field : fields) {
            ...
            if (ClassUtils.isSimpleType(field.getType())) {
                ...
                if (field.isAnnotationPresent(ExcelField.class)) {
                    ...
                    if (processorClazz != Void.class && ImportProcessor.class.isAssignableFrom(processorClazz)) {
                        try {
                ...
                fieldInfoMap.put(name, new ImportFieldInfo(currentClassChain, currentSetMethodChain, getMethodChain,
                        required, dateFormat, processor));
            } else {

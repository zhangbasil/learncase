...
import com.zhangbin.learncase.excel.bean.inter.LinkProcessor;
...
public class ExcelExportUtil {
	...
	private static <T> Workbook exportExcel(Class<T> clazz, List<T> list , ExcelFileType type, 
		...
		if(exportInfo == null ){
			try {
				exportInfo = initTargetClass(clazz, staticRowData);
		...
		try {
			for( ; i < page ; i++  ){
				...
				int dataSize = Math.abs(maxSheetSize - startRow);
				if(startSheetNum > 0){
	...
	private static ExportInfo initTargetClass(Class<?> clazz, Map<String, String> map)
			throws IntrospectionException {
		return ExcelUtils.getExportInfo(clazz, map);
//		ExportInfo exportInfo = ExcelUtils.getExportInfo(clazz, map);
